...
class XGBoost_Train_Predict:
    ...
    def __init__(self, X, Y, split_percent):
        # Define Instance variable
        self.X = X
        self.Y = Y
    ...
    def Split (self):
        ...
        X_train, X_test, Y_train, Y_test = train_test_split(self.X, self.Y, test_size=self.split_percent, random_state=10)
...
Converted_Dataset = data_pre.Drop_missed_Column(Converted_Dataset, 'Index_group') # To drop index_group" column
...
Converted_Dataset = data_pre.Transpose(Converted_Dataset)
...
# Just to show better vision on the trend of Prediction Shuffel the Dataset
X = X.sample(frac = 1)
Y = X.iloc[:,-1]
X = X.iloc[:,0:-1]

# Define XGBoost from Class of XGBoost_Train_Predict
XGB = XGBoost_Train_Predict(X, Y, 0.015)
X_train, X_test, Y_train, Y_test = XGB.Split()
# Create Model based on Train Data
model = XGB.Model_Fit()

# Predict X_Test
prediction_test = XGB.Model_Predict(model)
